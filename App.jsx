...
function StatusBadge({ status }) {
  const style = {
    未着手: { background: "#e5e7eb", color: "#6b7280" },
    進行中: { background: "#fef3c7", color: "#92400e" },
    完了: { background: "#d1fae5", color: "#065f46" },
  }[status];
  return <span style={{ ...style, padding: "2px 8px", borderRadius: 4, fontSize: 12, fontWeight: 600 }}>{status}</span>;
}
...
function HintAccordion({ hint }) {
  ...
  return (
    <div style={{ marginTop: 6, borderRadius: 6, border: "1px solid #d1fae5", overflow: "hidden" }}>
      <button onClick={() => setOpen(o => !o)}
        style={{ width: "100%", textAlign: "left", background: "#f0fdf4", border: "none", padding: "7px 10px", fontSize: 12, color: "#166534", cursor: "pointer", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        <span>{hint.short}</span>
        <span style={{ fontSize: 11, marginLeft: 8, flexShrink: 0 }}>{open ? "閉じる" : "詳細"}</span>
      </button>
      {open && (
        <div style={{ background: "#f0fdf4", padding: "6px 10px 10px", fontSize: 12, color: "#166534", lineHeight: 1.6, borderTop: "1px solid #d1fae5" }}>
          {hint.detail}
        </div>
      )}
    </div>
...
export default function App() {
  ...
  const [screen, setScreen] = useState(saved ? "list" : "template");
  ...
  // rooms / skipList のみ localStorage に保存（画面遷移状態は保存しない）
  useEffect(() => {
    try {
      if (rooms.length > 0) {
  ...
  const s = {
    wrap: { maxWidth: 480, margin: "0 auto", fontFamily: "sans-serif", fontSize: 15, color: "#111", padding: "0 0 80px" },
    header: { background: "#111", color: "#fff", padding: "14px 16px", fontWeight: 700, fontSize: 17, display: "flex", alignItems: "center", justifyContent: "space-between" },
    section: { padding: "12px 16px" },
    card: { background: "#fff", border: "1px solid #e5e7eb", borderRadius: 8, marginBottom: 10, overflow: "hidden" },
    cardHead: { padding: "10px 14px", display: "flex", alignItems: "center", justifyContent: "space-between", borderBottom: "1px solid #f3f4f6" },
    btn: { background: "#111", color: "#fff", border: "none", borderRadius: 6, padding: "12px 0", width: "100%", fontSize: 15, fontWeight: 600, cursor: "pointer", marginBottom: 8 },
    btnGray: { background: "#e5e7eb", color: "#374151", border: "none", borderRadius: 6, padding: "12px 0", width: "100%", fontSize: 15, fontWeight: 600, cursor: "pointer", marginBottom: 8 },
    btnSmall: { background: "#f3f4f6", color: "#374151", border: "none", borderRadius: 4, padding: "5px 10px", fontSize: 13, cursor: "pointer" },
    btnDanger: { background: "#fee2e2", color: "#b91c1c", border: "none", borderRadius: 4, padding: "5px 10px", fontSize: 13, cursor: "pointer" },
    input: { border: "1px solid #d1d5db", borderRadius: 6, padding: "10px 12px", fontSize: 15, width: "100%", boxSizing: "border-box" },
    inputNum: { border: "1px solid #d1d5db", borderRadius: 6, padding: "10px 6px", fontSize: 15, width: "70px", boxSizing: "border-box", textAlign: "center" },
    label: { fontSize: 12, color: "#6b7280", marginBottom: 3, display: "block" },
    progressBar: { height: 6, background: "#e5e7eb", borderRadius: 3, overflow: "hidden", margin: "6px 0" },
    floorLabel: { fontSize: 12, fontWeight: 700, color: "#6b7280", letterSpacing: 1, padding: "8px 16px 4px" },
    ...
    sheet: { background: "#fff", borderRadius: "16px 16px 0 0", padding: 20, width: "100%", maxWidth: 480, maxHeight: "70vh", overflowY: "auto" },
  };

  // ===== TEMPLATE =====
  if (screen === "template") return (
    <div style={s.wrap}>
      <div style={s.header}><span>内覧会ナビ — テンプレート選択</span></div>
      <div style={s.section}>
        <p style={{ color: "#6b7280", fontSize: 13, margin: "0 0 16px" }}>間取りに合ったテンプレートを選んでください。選択後にカスタマイズできます。</p>
        {Object.entries(TEMPLATES).map(([key, tpl]) => (
          <button key={key} onClick={() => selectTemplate(key)}
            style={{ width: "100%", border: "1px solid #e5e7eb", background: "#fff", cursor: "pointer", padding: "16px 14px", textAlign: "left", marginBottom: 10, display: "block", borderRadius: 8 }}>
            <div style={{ fontWeight: 700, fontSize: 16, marginBottom: 4 }}>{tpl.label}</div>
            <div style={{ fontSize: 13, color: "#6b7280" }}>{tpl.rooms.length}部屋 / {tpl.rooms.reduce((s, r) => s + r.items.length, 0)}項目</div>
          </button>
        ))}
      </div>
    </div>
  ...
  if (screen === "setup") return (
    <div style={s.wrap}>
      <div style={s.header}>
        <button onClick={() => setScreen("template")} style={{ background: "transparent", color: "#fff", border: "none", fontSize: 15, cursor: "pointer", padding: 0 }}>← テンプレート</button>
        <span>事前設定</span>
        <button onClick={() => { setScreen("list"); setCurrentRoomId(rooms[0]?.id); }}
          style={{ background: "#fff", color: "#111", border: "none", borderRadius: 6, padding: "6px 14px", fontWeight: 700, fontSize: 14, cursor: "pointer" }}>設定完了 →</button>
      </div>
      <div style={s.section}>
        <p style={{ color: "#6b7280", fontSize: 13, margin: "0 0 12px" }}>内覧会前に部屋と採寸項目をカスタマイズできます。</p>
              ...
              <div key={room.id} style={s.card}>
                ...
                {editingRoom === room.id && (
                  <div style={{ padding: 12 }}>
                    {room.items.map(item => (
                      <div key={item.id} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "7px 0", borderBottom: "1px solid #f3f4f6" }}>
                ...
                {editingRoom !== room.id && <div style={{ padding: "8px 14px", fontSize: 13, color: "#6b7280" }}>採寸項目 {room.items.length}件</div>}
              </div>
            ))}
          </div>
        ))}
  ...
  if (screen === "list") return (
    <div style={s.wrap}>
      <div style={s.header}>
        <span>部屋一覧</span>
        <button onClick={() => setScreen("setup")} style={{ background: "transparent", color: "#fff", border: "1px solid #fff", borderRadius: 6, padding: "4px 10px", fontSize: 13, cursor: "pointer" }}>設定に戻る</button>
      </div>
      <div style={s.section}>
        <div style={{ marginBottom: 16 }}>
          ...
          <div style={s.progressBar}><div style={{ height: "100%", width: progress + "%", background: "#111", transition: "width 0.3s" }} /></div>
        </div>
        {floors.map(floor => (
          <div key={floor}>
            <div style={s.floorLabel}>{FLOOR_LABELS[floor] || floor + "階"}</div>
              ...
              return (
                <div key={room.id} style={{ ...s.card, opacity: st === "完了" ? 0.6 : 1 }}>
                  <button onClick={() => goToRoom(room.id)} style={{ width: "100%", background: "none", border: "none", cursor: "pointer", padding: 0 }}>
                    <div style={{ ...s.cardHead, borderBottom: "none" }}>
                      <div style={{ textAlign: "left" }}>
                        <div style={{ fontWeight: 600, marginBottom: 2 }}>{room.name}</div>
                        <div style={{ fontSize: 13, color: "#6b7280" }}>{done}/{room.items.length} 項目</div>
                      </div>
        ...
        <div style={{ marginTop: 8 }}><button onClick={() => setScreen("done")} style={s.btnGray}>完了サマリーを見る</button></div>
      </div>
    </div>
  );
  ...
  if (screen === "measure" && currentRoom) {
    ...
    return (
      <div style={s.wrap}>
        ...
        <div style={{ height: 4, background: "#e5e7eb" }}><div style={{ height: "100%", width: progress + "%", background: "#22c55e" }} /></div>
        <div style={s.section}>
                ...
                <div style={{ padding: "10px 14px" }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 10 }}>
                    <button onClick={() => toggleDone(currentRoomId, item.id)}
                      style={{ width: 24, height: 24, borderRadius: 4, border: "2px solid #d1d5db", background: item.done ? "#111" : "#fff", cursor: "pointer", flexShrink: 0, display: "flex", alignItems: "center", justifyContent: "center" }}>
          ...
          <div style={{ marginTop: 16 }}>
            ...
            <button onClick={() => setShowRoomPicker(true)} style={s.btnGray}>部屋を選んで移動</button>
            <button onClick={() => setSkipConfirm(true)} style={{ ...s.btnGray, color: "#9ca3af" }}>この部屋をスキップ</button>
          </div>
        </div>
        ...
        {skipConfirm && (
          <div style={s.overlay} onClick={() => setSkipConfirm(false)}>
            <div style={s.sheet} onClick={e => e.stopPropagation()}>
              ...
              <div style={{ fontSize: 14, color: "#6b7280", marginBottom: 16 }}>「{currentRoom.name}」をスキップして次へ進みます。後から戻ることができます。</div>
        ...
        {showRoomPicker && (
          <div style={s.overlay} onClick={() => setShowRoomPicker(false)}>
            <div style={s.sheet} onClick={e => e.stopPropagation()}>
              ...
              {floors.map(floor => (
                <div key={floor}>
                  <div style={{ fontSize: 12, color: "#9ca3af", padding: "4px 0", fontWeight: 700 }}>{FLOOR_LABELS[floor]}</div>
                  {rooms.filter(r => r.floor === floor).map(room => (
                    <button key={room.id} onClick={() => goToRoom(room.id)}
                      style={{ width: "100%", textAlign: "left", padding: "12px 10px", border: "none", borderBottom: "1px solid #f3f4f6", background: room.id === currentRoomId ? "#f3f4f6" : "#fff", cursor: "pointer", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                      <span style={{ fontWeight: room.id === currentRoomId ? 700 : 400 }}>{room.name}</span>
  ...
  if (screen === "done") return (
    <div style={s.wrap}>
      <div style={s.header}><span>採寸完了</span></div>
      <div style={s.section}>
        <div style={{ textAlign: "center", padding: "24px 0" }}>
          <div style={{ fontSize: 28, fontWeight: 700, marginBottom: 4 }}>お疲れさまでした</div>
          <div style={{ color: "#6b7280", fontSize: 14 }}>内覧会の採寸が終わりました</div>
        </div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
            {[["採寸部屋数", `${doneRooms} / ${rooms.length}`], ["採寸項目数", `${doneItems} / ${totalItems}`], ["スキップ数", skipList.length], ["達成率", progress + "%"]].map(([k, v]) => (
              <div key={k} style={{ textAlign: "center" }}>
                <div style={{ fontSize: 13, color: "#6b7280", marginBottom: 4 }}>{k}</div>
        ...
        {skipList.length > 0 && (
          <div style={{ marginTop: 12 }}>
            <div style={{ fontSize: 13, color: "#6b7280", marginBottom: 8 }}>スキップした部屋</div>
            {skipList.map(id => { const r = rooms.find(x => x.id === id); return r ? <button key={id} onClick={() => goToRoom(id)} style={{ ...s.btnGray, marginBottom: 6 }}>{r.name} を入力する</button> : null; })}
          </div>
        )}
        <div style={{ marginTop: 16 }}>
          <button onClick={() => {
          ...
          <button onClick={() => setScreen("list")} style={s.btnGray}>部屋一覧に戻る</button>
          <button onClick={resetAll} style={{ ...s.btnGray, color: "#9ca3af" }}>最初からやり直す</button>
        </div>

        {exportText !== null && (
          <div style={s.overlay} onClick={() => setExportText(null)}>
            <div style={s.sheet} onClick={e => e.stopPropagation()}>
              <div style={{ fontWeight: 700, marginBottom: 8 }}>採寸データ</div>
              <div style={{ fontSize: 12, color: "#6b7280", marginBottom: 10 }}>下のテキストを長押しして全選択 → コピーしてください</div>
              <textarea
                readOnly
                value={exportText}
                onClick={e => e.target.select()}
                style={{ width: "100%", height: 240, fontSize: 13, fontFamily: "monospace", border: "1px solid #d1d5db", borderRadius: 6, padding: 10, boxSizing: "border-box", marginBottom: 12 }}
              />
